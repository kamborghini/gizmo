...
| B31 | low | The header's Customize button appears and disappears as you type in product search (products) | Left. Customise shows only when a page has two cards to arrange; that is the widget grid's own rule, and changing it for one search state would change it everywhere. |
...
| C-05 | low | Buttons and fields of different heights sit side by side (labels (To make) with a stale row; loans Chasing; size list inline editor and pager; crm Pipedrive panel) | Partly. A 28px control token (--control-h-md) now sizes every small button, table search and small field, so the Loans chasing field, the size editor, the mailbox address and the Team and forecast dates match the button beside them; the production queue's row buttons stay 32px as touch-friendly targets. |
...
| D10 | medium | One modal uses four different ways to save, identical switches behave differently, and the profile fields are filled a darker grey than a disabled field (settings modal) | Partly. Profile fields are white like every other field and the section rules are even; the two save models remain, and the section that saves at once now says so. |
...
| D14 | medium | The topbar title and the page heading give the same page two different names on 9 of 20 screens (shell topbar across screens) | Left. The page headings ('Store overview', 'Finance') differ from the nav names on purpose; renaming nine page titles is Cameron's call. |
